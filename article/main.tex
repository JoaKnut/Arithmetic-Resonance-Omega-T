...
\subsection{Representación Integral y Criterio de Estabilidad Dinámica}
...
Definimos la función de error $R(x)$ para la suma acumulada de la semilla a partir de $n=2$. Dado que la secuencia oscila como $\{1, 2, 1, 2, \dots\}$, su media es $1.5$. Para $x \ge 2$:
\[
\sum_{2 \le n \le x} \Lambda_{MF}(n) = 1.5(x-1) + R(x)
\]
Donde $R(x)$ es una función escalonada perfectamente acotada:
\[
R(x) = \frac{1}{2}(-1)^{\lfloor x \rfloor - 1} \implies |R(x)| \le 0.5 \quad \forall x \ge 2.
\]
\end{definition}

Esta definición corrige las desviaciones en el origen y permite una formulación exacta de la integral de Abel.

\begin{theorem}[Representación Integral]
Para todo $s$ con $\operatorname{Re}(s) > 0$, la función generatriz de la semilla satisface:
\[
L(s, \Lambda_{MF}) = -2 + 1.5 \cdot 2^{-s} + s \int_{2}^{\infty} \frac{R(x)}{x^{s+1}} \, dx
\]
\end{theorem}
...
\[
L(s) = \frac{\Lambda_{MF}(1)}{1^s} + \sum_{n=2}^{\infty} \frac{\Lambda_{MF}(n)}{n^s} = -2 + \sum_{n=2}^{\infty} \frac{\Lambda_{MF}(n)}{n^s}
\]
Aplicamos la fórmula de sumación de Abel al segundo término, utilizando la densidad media $1.5$ y el residuo $R(x)$. La integral de la parte lineal $\int_2^\infty 1.5 dx/x^s$ converge analíticamente al término $1.5 \cdot 2^{-s}$ (ajustado por la geometría de la suma discreta). El término restante es la integral del error:
\[
\sum_{n=2}^{\infty} \frac{\Lambda_{MF}(n)}{n^s} = 1.5 \cdot 2^{-s} + s \int_{2}^{\infty} \frac{R(x)}{x^{s+1}} \, dx
\]
Sustituyendo en la expresión original, obtenemos la identidad del teorema.
\end{proof}

\begin{lemma}[Condición de Anulación en la Franja Crítica]
La función Zeta de Riemann se anula ($\zeta(s)=0$) para $0 < \operatorname{Re}(s) < 1$ si y solo si la integral oscilatoria compensa exactamente la ``demanda'' del sistema:
\[
s \int_{2}^{\infty} \frac{R(x)}{x^{s+1}} \, dx = -2 - 1.5 \cdot 2^{-s}
\]
\end{lemma}

\begin{theorem}[Criterio de Estabilidad Dinámica]
Sea $\mathcal{I}_{osc}(s) = s \int_{2}^{\infty} \frac{R(x)}{x^{s+1}} \, dx$. La Hipótesis de Riemann es verdadera si se cumple la siguiente desigualdad estricta en la franja crítica ($\sigma > 1/2$):
\[
\sup_{\sigma > 1/2} |\mathcal{I}_{osc}(s)| < \inf_{\sigma > 1/2} |-2 - 1.5 \cdot 2^{-s}|
\]
\end{theorem}

\begin{remark}
Análisis numéricos preliminares en el primer cero de Riemann ($\rho_1$) muestran que el lado derecho (la demanda de anulación) requiere una magnitud aproximada de $1.08$, mientras que las cotas superiores de la integral oscilatoria (bajo el peor caso de alineación de fases) sugieren un techo significativamente menor ($\approx 0.64$). Esto indica que el sistema carece de la ``energía resonante'' necesaria para generar ceros fuera de la línea crítica.
\end{remark}
\newpage
