...
\section{Derivación Espectral de la Función $\pi(x)$}

En contraste con los enfoques probabilísticos que modelan la aparición de primos como ``lanzamientos de dados'' aleatorios (modelo de Cramér), este trabajo propone una construcción determinista basada en la dinámica de la paridad.

Argumentamos que la función de conteo $\pi(x)$ puede recuperarse mediante la descomposición espectral de una señal aritmética elemental, sin recurrir explícitamente a la criba de Eratóstenes.

\subsection{Estructura Algebraica de la Señal Base}
Para formalizar esta intuición, definimos la función generadora del sistema, que actúa como un ``púlsar'' binario.
...
Esta secuencia representa la proyección de la estructura multiplicativa sobre el cuerpo $\mathbb{F}_2$, preservando la ``carga'' necesaria para la reconstrucción. Sorprendentemente, esta señal trivial contiene toda la información necesaria para localizar los primos.

\begin{theorem}[Recurrencia de Von Mangoldt Generalizada]
Existe una función de tensión $\Lambda_{MF}$, análoga a la función clásica de Von Mangoldt, que satisface la relación de convolución exacta con la semilla:
\begin{equation}
...
\begin{proof}
Consideremos la serie de Dirichlet generatriz asociada a la semilla, $A(s) = \sum \alpha(n)n^{-s}$. Se conoce que esta serie corresponde a la identidad $(2-2^{-s})\zeta(s)$.
La derivada logarítmica de dicha función generatriz está dada por $-\frac{A'(s)}{A(s)}$.
En el anillo de las funciones aritméticas con la convolución de Dirichlet ($*$), la derivada logarítmica corresponde a una función $\Lambda_{MF}$ tal que:
\[ \Lambda_{MF} * \alpha = \alpha \cdot \ln \]
donde $(\cdot \ln)$ denota la multiplicación puntual por el logaritmo. Explícitamente:
\[ \sum_{d|n} \Lambda_{MF}(d) \alpha(n/d) = \alpha(n) \ln n \]
Aislando el término correspondiente a $d=n$ (donde el factor complementario es $\alpha(1)$), obtenemos la relación recursiva. Esto demuestra que $\Lambda_{MF}$ está unívocamente determinada por la estructura de paridad $\alpha$.
\end{proof}
...
La función exacta de conteo de números primos $\pi(x)$ emerge al filtrar los armónicos superiores:
\begin{equation}
...
\begin{tikzpicture}[scale=1, node distance=5cm]
    ...
    % Nodos
    \node [block] (input) {Enteros ($n$)};
    \node [block, right of=input, node distance=4cm] (parity) {Semilla $\alpha(n)$\\(Paridad)};
    \node [block, right of=parity, node distance=8cm] (tension) {Tensión $\Lambda_{MF}$\\(Convolución)};
    \node [block, below of=tension, node distance=2cm] (potential) {Potencial $J_{MFN}$};
    \node [block, left of=potential, node distance=8cm, fill=green!10] (output) {Conteo $\pi(x)$};

    % Conexiones
    \path [line] (input) -- (parity);
    \path [line] (parity) -- node[midway, above, font=\small] {Recurrencia} (tension);
    \path [line] (tension) -- node[midway, right, font=\small] {Integración} (potential);
    \path [line] (potential) -- node[midway, above, font=\small] {Inv. Möbius} (output);
    
    % Feedback loop decorativo
    \draw [dashed, ->, gray] (tension.north) to[out=150,in=30] node[midway, above, font=\scriptsize] {Feedback} (parity.north);

\end{tikzpicture}
\caption{Diagrama de flujo de la reconstrucción espectral. La información de los primos no se ``descubre'' mediante cribado, sino que se sintetiza transformando la señal de paridad elemental.}
\end{figure}

\begin{proof}
Por construcción, el potencial $J_{MFN}(x)$ no registra únicamente la ``energía'' de los números primos fundamentales $p$, sino que acumula armónicos de sus potencias $p^k$. Cada potencia contribuye con una intensidad atenuada por el factor geométrico $1/k$. Esta relación estructural se expresa formalmente como una suma infinita (que localmente es finita):
\[
J_{MFN}(x) = \pi(x) + \frac{1}{2}\pi(x^{1/2}) + \frac{1}{3}\pi(x^{1/3}) + \dots = \sum_{k=1}^{\infty} \frac{1}{k} \pi(x^{1/k}).
\]
El objetivo es ``limpiar'' la señal $J_{MFN}$ para recuperar la función pura $\pi(x)$. Para ello, invocamos la Fórmula de Inversión de Möbius generalizada. Este principio establece que si una función $f(x)$ se compone de una suma armónica de $g(x)$, entonces $g(x)$ se recupera mediante la suma alternada ponderada por la función de Möbius $\mu(k)$.

Aplicando la inversión a la serie anterior:
\[
\pi(x) = \sum_{k=1}^{\infty} \frac{\mu(k)}{k} J_{MFN}(x^{1/k}).
\]
La suma superior es teóricamente infinita, pero en la práctica se trunca naturalmente. Dado que $J_{MFN}(y) = 0$ para $y < 2$, los términos solo son no nulos mientras $x^{1/k} \ge 2$. Despejando $k$, obtenemos el límite superior de la suma $k \le \log_2 x$. Esto convierte la serie en una identidad aritmética finita y exacta.
\end{proof}

\begin{remark}
Es relevante notar que esta formulación es puramente aritmética. A diferencia de las aproximaciones asintóticas derivadas del Teorema de los Números Primos (como el Logaritmo Integral $Li(x)$), esta identidad no conlleva un término de error inherente. La precisión de $\pi(x)$ depende exclusivamente de la capacidad computacional para resolver la convolución de la semilla $\alpha$ y calcular $J_{MFN}$.
\end{remark}
